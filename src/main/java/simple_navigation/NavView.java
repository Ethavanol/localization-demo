...
public class NavView extends GridWorldView {
    ...
    public void draw(Graphics g, int x, int y, int object) {
        ...
        if (object == NavModel.POSSIBLE_LOC){
            ...
            if(this.navModel.gridPossibleObjects.containsKey(new Location(x,y))){
                ...
                if(this.navModel.gridPossibleObjects.get(new Location(x,y)).contains(NavModel.POSSIBLE_LOC_GOAL)){
        ...
        if (object == NavModel.POSSIBLE_LOC_GOAL){
            g2.setColor(Color.blue);
            g2.setStroke(new BasicStroke(5));
            g2.drawRect(x * cellSizeW + 3, y * cellSizeH + 3, cellSizeW - 6, cellSizeH - 6);
            if(this.navModel.gridPossibleObjects.containsKey(new Location(x,y))){
                if(this.navModel.gridPossibleObjects.get(new Location(x,y)).contains(NavModel.POSSIBLE_LOC)){
                    g2.setColor(Color.red);
                    g2.drawOval(x * cellSizeW + 3, y * cellSizeH + 3, cellSizeW - 6, cellSizeH - 6);
                }
            }
