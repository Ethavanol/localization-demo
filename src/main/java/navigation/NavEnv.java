...
public class NavEnv extends Environment implements MapEventListener {
    ...
    public void init(String[] args){
        ...
        NavModel.loadFromFile(MapType.LOCALIZATION_5x5);

        agentName = args[1];

        view = new NavView(MapType.LOCALIZATION_5x5);
        model = view.getModel();
    ...
    public boolean executeAction(String ag, Structure action) {
        ...
        try{
            ...
            if (action.getFunctor().equals("move")) {
                ...
                switch (dir){
                    case "up":
                        result = model.move(NavModel.Move.UP, agId);
                        break;
                    case "down":
                        result = model.move(NavModel.Move.DOWN, agId);
                        break;
                    case "left":
                        result = model.move(NavModel.Move.LEFT, agId);
                        break;
                    case "right":
                        result = model.move(NavModel.Move.RIGHT, agId);
                        break;
                }
            }
        } catch (Exception e) {
