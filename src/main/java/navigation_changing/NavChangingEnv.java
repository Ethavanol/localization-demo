...
public class NavChangingEnv extends Environment implements MapEventListener {

    private HashMap<Integer, Queue<MapEvent>> mapEventQueue;

    public static final Literal ag = Literal.parseLiteral("at(goal)");

    static Logger logger = Logger.getLogger(NavChangingEnv.class.getName());

    private NavChangingModel model;
    ...
    public void init(String[] args){
        ...
        model = NavChangingModel.loadFromFile(MapType.LOCALIZATION_5x5);

        if(args.length > 1 && args[1].equals("gui")){
            NavChangingView navView = new NavChangingView(model);
    ...
    public NavChangingModel getModel() {
        return model;
    }
