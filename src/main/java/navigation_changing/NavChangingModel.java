...
public class NavChangingModel extends GridWorldModel {
    ...
    public NavChangingModel(int w, int h, int nbAgs) {
        super(w, h, nbAgs);
        this.width = w;
        this.height = h;
        this.nbAgts = nbAgs;
    ...
    public NavChangingModel(LocalizationMap map, MapType mapType) {
        ...
        List<Location> agentStarts = map.getAgentStart();
        int nbAgents = map.getNbAgts();
        int agentStartsSize = agentStarts.size();
        for (int i = 0; i < nbAgents; i++) {
            this.setAgPos(i, agentStarts.get(i));
        }
//        randomAgentSpawn(this.width,this.height,this.nbAgts);
    }

    public static NavChangingModel loadFromFile(MapType mapType) {
        Gson gson = new Gson();
        ...
        return new NavChangingModel(map, mapType);
    }
